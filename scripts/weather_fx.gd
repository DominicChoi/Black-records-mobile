...
func set_environment(hour: float, next_weather: String) -> void:
    time_of_day = hour
...
func _draw() -> void:
    var dusk: float = _night_amount(time_of_day)
    _draw_haze(dusk)
    if weather == "mist":
        _draw_mist(transition, dusk)
    elif weather == "drizzle":
        _draw_mist(0.48, dusk)
        _draw_drizzle(transition)
    elif weather == "overcast":
        _draw_cloud_veil(transition, dusk)
    _draw_vignette(dusk)
...
func _draw_haze(night_amount: float) -> void:
    ...
    if night_amount > 0.01:
        draw_rect(Rect2(0, 0, size.x, size.y), Color(0.05, 0.09, 0.15, 0.055 * night_amount), true)

func _draw_mist(alpha_scale: float, night_amount: float) -> void:
    var base_alpha: float = lerpf(0.035, 0.065, night_amount) * alpha_scale
