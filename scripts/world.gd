...
func _draw_lanterns() -> void:
    for p in lanterns:
        var glow: float = 19.0 + sin(t * 2.0 + p.x) * 2.0
        draw_circle(p, glow * 1.8, Color(1.0,0.68,0.34,0.025))
        draw_circle(p, glow, Color(1.0,0.68,0.34,0.07))
        draw_line(p + Vector2(0,-2), p + Vector2(0,35), Color("2c2925"), 4.0)
        draw_circle(p, 5.0, Color("e8ad63"))
        draw_circle(p + Vector2(-1,-1), 2.0, Color("ffe3a6"))
